...
| TeX 发行版 | TeX Live / MacTeX | 提供 `xelatex`、`bibtex`、`latexmk`、宏包 |
...
Windows 用户可以安装 Git for Windows；macOS 用户可以先安装 Xcode Command Line Tools 或 Homebrew，再使用 `git`：

![Git for Windows 下载页面](/images/posts/git-for-windows-download.png)
...
### 2. 安装 TeX Live / MacTeX

#### Windows

Windows 推荐使用 TeX Live 官方 `install-tl-windows.exe` 或清华 TUNA 镜像。安装时注意两点：

![TeX Live Windows 安装说明页面](/images/posts/texlive-windows-page.png)

- 安装目录尽量使用纯英文路径，例如 `C:\texlive\2026`，不要放在带中文或特殊字符的路径下。
...
macOS 推荐安装 MacTeX。它是面向 macOS 的 TeX Live 发行版，安装后会提供 `xelatex`、`bibtex`、`latexmk`、TeX Live Utility 等工具。

![MacTeX 下载页面](/images/posts/mactex-download-page.png)

常用安装方式有两种：

```bash
# 方式 A：下载安装包
# 打开 https://tug.org/mactex/ 下载 MacTeX.pkg 后双击安装

# 方式 B：如果已经安装 Homebrew
...
如果 `which xelatex` 没有输出，通常是 shell 没加载 TeX Live 路径。新开一个终端一般可以解决；如果还不行，检查 `/Library/TeX/texbin` 是否在 PATH 中。

macOS 上中文字体通常可以先使用系统字体；如果模板配置里写了 `fontset=windows`，需要改成适合 macOS 的字体配置，或者按 ThuThesis 文档指定具体字体。不要把 Windows 的 `SimSun`、`Microsoft YaHei` 配置直接搬到 macOS。

### 3. 安装 VS Code 和插件

先安装 VS Code，再安装 LaTeX Workshop 插件。

![VS Code 下载页面](/images/posts/vscode-download-page.png)
...
![LaTeX Workshop 插件页面](/images/posts/vscode-latex-workshop-marketplace.png)
...
```json
{
  "latex-workshop.latex.build.fromFolder": "rootFile",
...
如果 VS Code 在 `"cwd": "%DIR%"` 下画黄线，直接删掉这一行即可。LaTeX Workshop 当前的 `tools` 配置不需要 `cwd` 字段；上面的 `"latex-workshop.latex.build.fromFolder": "rootFile"` 已经明确让编译命令在根文件所在目录执行。

Windows 上如果都装在本机，可以打开 PowerShell 一次性确认这些命令是否在 PATH 中：

![Windows 终端环境验证示例](/images/posts/windows-terminal-env-check.svg)

## 四、获取 ThuThesis 模板

ThuThesis 官方建议新手优先下载发布版，而不是直接用 GitHub `master` 开发版。

![ThuThesis GitHub 仓库页面](/images/posts/thuthesis-github-repo.png)
...
OpenCode 在 Windows、macOS、Linux 上都能使用。Windows 用户更推荐走 WSL；macOS 用户可以直接在系统终端中安装和运行。

![OpenCode 文档页面](/images/posts/opencode-docs-page.png)
...
macOS 上建议把论文项目放在用户目录下的普通英文路径，例如 `~/code/my-thesis`。如果项目在云同步目录或外接盘中，遇到文件监听、权限或同步冲突时，优先复制到本地普通目录再写作。
...
![OpenCode Go 订阅页面](/images/posts/opencode-go-subscription-page.png)
...
连接和选模型的界面可以参考下面这张图。实际模型列表会随 OpenCode Go 更新而变化，按当时 TUI 里显示的可用模型选择即可。

![OpenCode Go 连接和模型选择示例](/images/posts/opencode-go-connect-tui.svg)
...
- macOS 本地编译需要使用 macOS 可用字体，不要照抄 Windows 的 `SimSun`、`Microsoft YaHei`。
- Linux/WSL 编译：不要假设有 Windows 或 macOS 字体，要安装对应中文字体或改模板字体配置。
- 不要在 Windows、macOS、WSL/Linux 之间混用同一套字体配置。
...
1. 下载 ThuThesis 发布版。
2. 本地安装 TeX Live 或 MacTeX、VS Code、LaTeX Workshop。
3. 保证原始模板能编译。
4. 建 Git 仓库并提交初始版本。
5. 拆分章节、图片、参考文献。
6. 安装 OpenCode。
